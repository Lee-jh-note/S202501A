...
<body>
    ...
    <form name="frm">
        <table>
            <tr><th>제목</th><td>
                    <input type="text" name="title" 			required="required"	 placeholder="발주_YYYYMMDD_거래처명"></td></tr>
            <tr><th>매입일자</th><td>
                    <input type="date" name="purchase_date" 	required="required"  value="${sysdate}" readonly></td></tr>
            <tr><th>요청배송일</th><td>
                    <input type="date" name="req_delivery_date" required="required"></td></tr>
            <tr><th>담당자</th><td>
                    <!-- 화면에 이름 표시 -->
			        <span>${emp_name}</span>
			        <!-- emp_no 값을 숨겨서 전송 -->
			        <input type="hidden" name="emp_no" value="${emp_no}">
                    <%-- <select name="emp_no" required="required">
			            <option value="${emp_no}" selected>${emp_name}</option>
			        </select> --%>
            		<%-- <select name="emp_no">
            ...
            <tr><th>비고</th><td>
                    <input type="text" name="remarks"  value="${purchase.remarks}"></td></tr>
            <tr><td colspan="2"><button type="button" id="addRow">+ 추가</button></td></tr>
        </table>
        <!-- 동적으로 추가될 입력칸을 담을 컨테이너 -->
		<table>
		    <thead>
		        <tr>
		            <th>품목명</th><th>단가</th><th>수량</th><th>총금액</th><th>삭제</th>
		        </tr>
		    </thead>
       	 ...
       	 <input  type="button" id="btn" value="발주서 등록" onclick="insertPurchase()">
       	 <a href=/listPurchase><input  type="button" id="btn_x" value="취소"></a><p>
       	 </form>
       	<!--  <input  type="button" id="btn" value="발주서 물품 등록" onclick="insertDetailPurchase()"><p> -->
