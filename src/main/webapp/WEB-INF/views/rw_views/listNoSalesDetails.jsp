...
function exportToExcel() {
    let table = document.getElementById("noSalesTable"); // 테이블 요소 가져오기
...
function printSelectedRows() {
    ...
    let headers = document.querySelectorAll("#noSalesTable th"); // 원본 테이블 헤더 가져오기
    headers.forEach(th => {
			...
			<div id="content">
				...
				<div class="list-wrapper">
					<div class="list-header">
						<div>
							<div class="list-submenu">믈류 관리 > 미출고 조회</div>
						...
						<div class="list-buttons">
							<!-- 엑셀 다운로드 및 인쇄 기능 -->
							<a href="/excel/listNoSalesDetailsExcel?startDate=${param.startDate}&endDate=${param.endDate}&client_name=${param.client_name}">
					...
					<div class="list-header2">
						...
						<div class="list-search-filters">
							...
							<!-- 검색 필터 -->
							<form action="listNoSalesDetails" method="get" style="display: flex; gap: 10px; align-items: center;">
					...
					<table class="list-table" id="noSalesTable">
						<thead>
							<tr>
						...
						<tbody>
							<c:forEach var="salesDetails" items="${listNoSalesDetails}" varStatus="status">
								<tr>
									<td><input type="checkbox" class="selectRow"></td>
									<td>${(page.total - ((page.currentPage - 1) * page.rowPage)) - status.index}</td>
									<td>${salesDetails.sales_date}</td>
									<td><a href="infoNoSalesDetails?sales_date=${salesDetails.sales_date}&client_no=${salesDetails.client_no}"> ${salesDetails.client_name} </a></td>
							...
							<!-- 데이터가 없을 경우 -->
							<c:if test="${empty listNoSalesDetails}">
								<tr>
									<td colspan="10">미출고 데이터가 없습니다.</td>
								</tr>
							</c:if>
					...
					<div style="text-align: center; margin-top: 20px;">
						<c:if test="${page.startPage > page.pageBlock}">
							<a href="listNoSalesDetails?startDate=${param.startDate}&endDate=${param.endDate}&client_Name=${param.client_Name}&status=${param.status}&currentPage=${page.startPage - page.pageBlock}">[이전]</a>
						</c:if>
						<c:forEach var="i" begin="${page.startPage}" end="${page.endPage}">
							<a href="listNoSalesDetails?startDate=${param.startDate}&endDate=${param.endDate}&client_Name=${param.client_Name}&status=${param.status}&currentPage=${i}">[${i}]</a>
						</c:forEach>
						<c:if test="${page.endPage < page.totalPage}">
							<a href="listNoSalesDetails?startDate=${param.startDate}&endDate=${param.endDate}&client_Name=${param.client_Name}&status=${param.status}&currentPage=${page.startPage + page.pageBlock}">[다음]</a>
						</c:if>
					</div>
