...
function exportToExcel() {
    let table = document.getElementById("salesTable"); // 테이블 요소 가져오기
			...
			<div id="content">
				...
				<div class="list-wrapper">
					<div class="list-header">
						<div>
							<div class="list-submenu">발주/수주 관리 > 수주 조회</div>
						...
						<div class="list-buttons">
							<!-- 엑셀 다운로드 및 인쇄 기능 -->
							<a href="/excel/listSalesExcel?startDate=${param.startDate}&endDate=${param.endDate}&client_name=${param.client_name}&status=${param.status}">
						...
						<div class="list-search-filters">
                            ...
                            <!-- 처리상태(status) -->								
								...
								<select name="status">
									...
									<option value="2" ${param.status == '2' ? 'selected' : ''}>완료</option>
								</select>
						...
						<tbody>
							<c:forEach var="sales" items="${listSales}" varStatus="status">
								<tr>
									...
									<td>${sales.status}</td>
									<td>${sales.req_delivery_date.substring(0,10)}</td>
								</tr>
							</c:forEach>
					...
					<div style="text-align: center; margin-top: 20px;">
						<c:if test="${page.startPage > page.pageBlock}">
							<a href="listSales?startDate=${param.startDate}&endDate=${param.endDate}&client_Name=${param.client_Name}&status=${param.status}&currentPage=${page.startPage - page.pageBlock}">[이전]</a>
						</c:if>
						<c:forEach var="i" begin="${page.startPage}" end="${page.endPage}">
							<a href="listSales?startDate=${param.startDate}&endDate=${param.endDate}&client_Name=${param.client_Name}&status=${param.status}&currentPage=${i}">[${i}]</a>
						</c:forEach>
						<c:if test="${page.endPage < page.totalPage}">
							<a href="listSales?startDate=${param.startDate}&endDate=${param.endDate}&client_Name=${param.client_Name}&status=${param.status}&currentPage=${page.startPage + page.pageBlock}">[다음]</a>
						</c:if>
					</div>
