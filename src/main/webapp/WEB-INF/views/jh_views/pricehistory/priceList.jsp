    ...
    <table class="table table-bordered table-striped">
        ...
        <c:forEach var="price" items="${priceList}">
            <tr>
                ...
                <td>
                    <c:choose>
                        <c:when test="${price.sale_or_purchase == 1}">판매</c:when>
                ...
                <td>
                    <!-- 가격 수정 버튼 -->
                    <c:if test="${price.to_date == '99/12/31'}">
                        <form action="/Prod/ProdModify" method="get">
                                <%--                            <input type="hidden" name="id" value="${price.id}"/>--%>
                                <%--                            <input type="hidden" name="product_name" value="${price.product_name}"/>--%>
                            <input type="hidden" name="productNo" value="${price.product_no}"/>
    ...
    <div class="d-flex justify-content-center">
        <c:forEach var="i" begin="${page.startPage}" end="${page.endPage}">
            <a href="/Prod/PriceList?name=${SName}&type=${SType}&currentPage=${i}"
               class="btn btn-outline-secondary btn-sm mx-1">${i}</a>
        </c:forEach>
    </div>
