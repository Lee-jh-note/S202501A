...
<body>
	...
	<table>
		...
		<c:forEach var="purchase_details" items="${listPurchaseDetail}">
			<tr>
				<td>${num}</td>
	...
	<c:if test="${page.startPage > page.pageBlock }">
				<!-- 다른점: 7. 페이지 이동 -->
		<a href="listPurchaseDetail?currentPage=${page.startPage-page.pageBlock}">[이전]</a>
	</c:if>
	<c:forEach var="i" begin="${page.startPage}" end="${page.endPage}">
				<!-- 다른점: 7. 페이지 이동 -->
		<a href="listPurchaseDetail?currentPage=${i}">[${i}]</a>
	</c:forEach>
	<c:if test="${page.endPage < page.totalPage }">
				<!-- 다른점: 7. 페이지 이동 -->
		<a href="listPurchaseDetail?currentPage=${page.startPage+page.pageBlock}">[다음]</a>
	</c:if>	
	</div>
