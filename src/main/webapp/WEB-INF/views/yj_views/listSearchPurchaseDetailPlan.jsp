...
<body>
	...
	<table>
		<tr><th >NO</th><th>매입일자</th><th>요청배송일</th><th>거래처명</th><th>발주 담당자</th><th>입고 담당자</th><th>상품수</th><th>총수량</th><th>총금액</th><th>처리상태</th></tr>
	...
	<c:if test="${page.startPage > page.pageBlock }">
			<!-- 다른점: 7. 페이지 이동 -->
	  <a href="searchPurchaseDetailPlan?startDate=${param.startDate}&endDate=${param.endDate}&client_name=${param.client_name}&req_delivery_date=${param.req_delivery_date}&currentPage=${page.startPage-page.pageBlock}">[이전]</a>
	</c:if>
	<c:forEach var="i" begin="${page.startPage}" end="${page.endPage}">
			<!-- 다른점: 7. 페이지 이동 -->
	  <a href="searchPurchaseDetailPlan?startDate=${param.startDate}&endDate=${param.endDate}&client_name=${param.client_name}&req_delivery_date=${param.req_delivery_date}&currentPage=${i}">[${i}]</a>
	</c:forEach>
	<c:if test="${page.endPage < page.totalPage }">
			<!-- 다른점: 7. 페이지 이동 -->
	  <a href="searchPurchaseDetailPlan?startDate=${param.startDate}&endDate=${param.endDate}&client_name=${param.client_name}&req_delivery_date=${param.req_delivery_date}&currentPage=${page.startPage+page.pageBlock}">[다음]</a>
	</c:if>
	</div>
