...
<body>
	...
	<table>
		...
		<c:forEach var="purchase_details" items="${listPurchaseDetailNo}">
			<tr>
				<td>${num}</td>
				...
				<td>${purchase_details.emp_name}</td><td>${purchase_details.product_count}</td>
	...
	<c:if test="${page.startPage > page.pageBlock }">
				<!-- 다른점: 7. 페이지 이동 -->
		<a href="listPurchaseDetailNo?currentPage=${page.startPage-page.pageBlock}">[이전]</a>
	</c:if>
	<c:forEach var="i" begin="${page.startPage}" end="${page.endPage}">
				<!-- 다른점: 7. 페이지 이동 -->
		<a href="listPurchaseDetailNo?currentPage=${i}">[${i}]</a>
	</c:forEach>
	<c:if test="${page.endPage < page.totalPage }">
				<!-- 다른점: 7. 페이지 이동 -->
		<a href="listPurchaseDetailNo?currentPage=${page.startPage+page.pageBlock}">[다음]</a>
	</c:if>	
	</div>
