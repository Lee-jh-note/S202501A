...
<body>
	...
	<!-- 다른점: 3. 액션 이동 -->
    <form action="searchPurchaseDetailNo">
	...
	<c:if test="${page.startPage > page.pageBlock }">
			<!-- 다른점: 7. 페이지 이동 -->
	  <a href="searchPurchaseDetailNo?startDate=${param.startDate}&endDate=${param.endDate}&product_name=${param.product_name}&client_name=${param.client_name}&emp_name=${param.emp_name}&currentPage=${page.startPage-page.pageBlock}">[이전]</a>
	</c:if>
	<c:forEach var="i" begin="${page.startPage}" end="${page.endPage}">
			<!-- 다른점: 7. 페이지 이동 -->
	  <a href="searchPurchaseDetailNo?startDate=${param.startDate}&endDate=${param.endDate}&product_name=${param.product_name}&client_name=${param.client_name}&emp_name=${param.emp_name}&currentPage=${i}">[${i}]</a>
	</c:forEach>
	<c:if test="${page.endPage < page.totalPage }">
			<!-- 다른점: 7. 페이지 이동 -->
	  <a href="searchPurchaseDetailNo?startDate=${param.startDate}&endDate=${param.endDate}&product_name=${param.product_name}&client_name=${param.client_name}&emp_name=${param.emp_name}&currentPage=${page.startPage+page.pageBlock}">[다음]</a>
	</c:if>
	</div>
